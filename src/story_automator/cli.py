...
from .commands.tmux import cmd_check_session, cmd_codex_status_check, cmd_heartbeat_check, cmd_monitor_session, cmd_tmux_status_check, cmd_tmux_wrapper
...
def _cmd_parse_epic(args: list[str]) -> int:
    ...
    epic_num = _arg_value(args, "--epic-num")
    if not epic_file:
        print_json({"ok": False, "error": "epic_file_not_found"})
        return 1
    try:
        print_json(parse_epic_file(epic_file, epic_num))
        return 0
...
def _cmd_epic_complete(args: list[str]) -> int:
    ...
    epic_num = _arg_value(args, "--epic-num") or ""
    if not epic:
        print_json({"ok": False, "error": "epic_file_not_found"})
        return 1
    try:
        print_json(epic_complete(epic, range_csv, epic_num))
        return 0
    except FileNotFoundError:
        print_json({"ok": False, "error": "epic_file_not_found"})
        return 1
