...
from .project_config import load_project_config, reviewer_bridge
from .utils import ensure_dir, get_project_root, iso_now, md5_hex8, read_text, write_atomic

VALID_TOP_LEVEL_KEYS = {"version", "snapshot", "runtime", "workflow", "steps", "models", "bmadPaths"}
...
def load_effective_policy(project_root: str | None = None, *, resolve_assets: bool = True) -> dict[str, Any]:
    ...
    # Precedence: bundled default < per-project story-automator.yaml < explicit
    # policy JSON override. The reviewer bridge (which review skill the review
    # step invokes) is sourced from story-automator.yaml.
    config_override = _project_reviewer_override(root)
    policy = _deep_merge(_deep_merge(bundled, config_override), override)
...
def bundled_skill_root(project_root: str | Path | None = None) -> Path:
    # The bundle root is the skill directory that ships this package — derived
    # from this file's location so it is independent of the skill's name
    # (global `story-automator`, project-local `lz-story-automator`, etc.).
    # __file__ == <skill_root>/src/story_automator/core/runtime_policy.py
    for parent in Path(__file__).resolve().parents:
        if (parent / "data" / "orchestration-policy.json").is_file():
            return parent
    # Fallback: a project-local install bundled under the project's skills dir.
    if project_root:
        skills_root = Path(project_root).resolve() / ".claude" / "skills"
        for name in ("story-automator", "lz-story-automator"):
...
def _project_reviewer_override(project_root: Path) -> dict[str, Any]:
    """Build a policy fragment overriding the review step's bridge skill from
    the per-project story-automator.yaml. Returns ``{}`` when no config file."""
    cfg = load_project_config(project_root)
    if not cfg:
        return {}
    bridge = reviewer_bridge(project_root, cfg)
    review_override: dict[str, Any] = {"label": bridge, "assets": {"skillName": bridge}}
    contract_rel = Path(".claude") / "skills" / bridge / "contract.json"
    if (project_root / contract_rel).is_file():
        review_override["success"] = {"contractFile": str(contract_rel)}
    return {"steps": {"review": review_override}}
...
def _validate_policy_shape(policy: dict[str, Any]) -> None:
    ...
    _validate_models_block(policy)
    _validate_bmad_paths(policy)
    ...
    for name, contract in steps.items():
        ...
        candidates = assets.get("skillCandidates")
        if candidates is not None and (
            not isinstance(candidates, list) or not all(isinstance(item, str) for item in candidates)
        ):
            raise PolicyError(f"skillCandidates for {name} must be a string array")


def bmad_paths(policy: dict[str, Any]) -> dict[str, str]:
    """The declared BMAD path conventions (config, sprint-status, artifact dirs).

    A single declarative source of truth so a BMAD layout change is one edit and
    is verifiable by `doctor`. Values are project-root-relative strings.
    """
    raw = _expect_optional_dict(policy, "bmadPaths")
    return {str(k): str(v) for k, v in raw.items() if isinstance(v, str) and v.strip()}


def _validate_bmad_paths(policy: dict[str, Any]) -> None:
    paths = _expect_optional_dict(policy, "bmadPaths")
    for key, value in paths.items():
        if not isinstance(value, str):
            raise PolicyError(f"bmadPaths.{key} must be a string")


def _validate_models_block(policy: dict[str, Any]) -> None:
    """Validate the optional top-level ``models`` block (tier tables + Fable
    usage-fallback config). Absent block is valid; defaults live in
    ``core/model_select.py``."""
    models = _expect_optional_dict(policy, "models")
    if not models:
        return
    tiers = _expect_optional_nested_dict(models, "tiers", "models")
    for table_name in ("normal", "fableExhausted"):
        table = _expect_optional_nested_dict(tiers, table_name, "models.tiers")
        for key, value in table.items():
            if not isinstance(value, str):
                raise PolicyError(f"models.tiers.{table_name}.{key} must be a string")
    fallback = _expect_optional_nested_dict(models, "fableFallback", "models")
    threshold = fallback.get("thresholdPercent")
    if "thresholdPercent" in fallback and (isinstance(threshold, bool) or not isinstance(threshold, (int, float))):
        raise PolicyError("models.fableFallback.thresholdPercent must be a number")
    if "scopedModelName" in fallback and not isinstance(fallback.get("scopedModelName"), str):
        raise PolicyError("models.fableFallback.scopedModelName must be a string")
    review_steps = models.get("reviewSteps")
    if review_steps is not None and (
        not isinstance(review_steps, list) or not all(isinstance(item, str) for item in review_steps)
    ):
        raise PolicyError("models.reviewSteps must be a string array")
    for str_key in ("usageCachePath", "defaultComplexity"):
        if str_key in models and not isinstance(models.get(str_key), str):
            raise PolicyError(f"models.{str_key} must be a string")
...
def resolve_skill_name(assets: dict[str, Any], skills_root: Path) -> tuple[str, list[str]]:
    """Resolve the concrete BMAD skill for a step from its candidate list.

    This is the capability-adapter seam: the policy names a step's skill via an
    ordered ``skillCandidates`` list (first whose ``SKILL.md`` exists wins), so a
    BMAD rename is a one-line edit here rather than a break in the orchestration.
    ``skillName`` is appended as the final fallback, keeping legacy policies and
    the review-bridge override working. When nothing resolves, the preferred name
    (``skillName`` or the first candidate) is returned so the caller's
    missing-required-asset error names something sensible.

    Returns ``(chosen, candidates)``.
    """
    candidates: list[str] = []
    raw = assets.get("skillCandidates")
    if isinstance(raw, list):
        candidates = [str(c).strip() for c in raw if str(c).strip()]
    skill_name = str(assets.get("skillName") or "").strip()
    if skill_name and skill_name not in candidates:
        candidates.append(skill_name)
    for candidate in candidates:
        if (skills_root / candidate / "SKILL.md").is_file():
            return candidate, candidates
    preferred = skill_name or (candidates[0] if candidates else "")
...
def _resolve_step_assets(step: str, assets: dict[str, Any], project_root: Path) -> dict[str, str]:
    ...
    skill_name, _candidates = resolve_skill_name(assets, skills_root)
    if not skill_name:
        raise PolicyError(f"missing skillName/skillCandidates for {step}")
    skill_dir = _ensure_within(skills_root / skill_name, skills_root, f"skill for {step}")
