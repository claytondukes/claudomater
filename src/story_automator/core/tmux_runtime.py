...
from .auto_answer import match_known_prompt
from .utils import (
    atomic_write,
    command_exists,
    file_exists,
...
# Story 24-10 — pane-watcher debounce window. Same literal prompt
# matched within this window is treated as a re-render of the
# already-answered state, not a new prompt to answer.
AUTO_ANSWER_DEBOUNCE_SECONDS = 30
AUTO_ANSWER_ENV_VAR = "AUTO_ANSWER_ELICITATION"
AUTO_ANSWER_STATE_DOC_ENV_VAR = "STATE_DOC_FILE"
...
def agent_cli(agent: str) -> str:
    return "codex exec" if agent == "codex" else "claude --permission-mode bypassPermissions"
...
def spawn_session(
    ...
    mode: str | None = None,
    *,
    auto_answer_elicitation: bool = False,
    state_doc_file: str = "",
) -> tuple[str, int]:
...
def _spawn_runner(
    ...
    *,
    ...
    create_out, create_code = run_cmd(
        ...
        "-e",
        f"STORY_AUTOMATOR_SESSION={session}",
        "-e",
        f"AI_AGENT={selected_agent}",
        "-e",
        f"{AUTO_ANSWER_ENV_VAR}={'true' if auto_answer_elicitation else 'false'}",
        "-e",
        f"{AUTO_ANSWER_STATE_DOC_ENV_VAR}={state_doc_file}",
        "-e",
        "CLAUDECODE=",
        "-e",
        "BASH_ENV=",
...
def _runner_session_status(
    ...
    *,
    ...
    # Story 24-10 — pane-watcher: detect and auto-answer known
    # interactive y/n prompts that would otherwise stall the spawned
    # session indefinitely. Gated on the `AUTO_ANSWER_ELICITATION`
    # tmux env var set at spawn time when the orchestration state
    # doc's overrides.autoAnswerElicitation is true. No-op when not
    # gated on and no whitelist match; log-only when gated off but
    # a match is detected.
...
def _runner_claude_prompt_completed(
    ...
    if prompt_visible != "true":
        return False
    turn_stamped = bool(str(state.get("turnCompletedAt") or "").strip())
    if not turn_stamped and not _claude_completion_marker_present(capture) and not _idle_pane_stable(paths, state, capture):
        return False
...
def _stable_idle_lines(capture: str) -> str:
    """Pane content minus the always-ticking footer chrome.

    The status footer updates every minute ("session 14m", rate-limit
    resets, ctx %), so raw pane hashes never stabilize. Strip those
    lines before hashing; everything else is frozen once a turn ends.
    """
    kept: list[str] = []
    for line in capture.splitlines():
        if re.search(r"resets in|\bctx\b|session \d+[hm]|auto-compact|shift\+tab|for agents|^\s*[─╌╍—-]{5,}\s*$", line):
            continue
        kept.append(line.rstrip())
    return "\n".join(kept).strip()


def _idle_pane_stable(paths: SessionPaths, state: dict[str, object], capture: str) -> bool:
    """Completion fallback that needs no verb regex (user idea, 2026-07-03):
    when the pane is idle-at-prompt and its content (footer excluded) has
    not changed since the previous check ≥60s ago, the turn is done — a
    working session always mutates the pane (spinner, elapsed timer,
    streamed output). Guards against future TUI verb/format changes
    stranding sessions the way "Crunched"/"Sautéed" did.
    """
    import hashlib
    digest = hashlib.sha256(_stable_idle_lines(capture).encode("utf-8")).hexdigest()[:16]
    prev_digest = str(state.get("idlePaneHash") or "")
    prev_at = str(state.get("idlePaneAt") or "")
    if digest == prev_digest and prev_at and _seconds_since(prev_at) >= 60:
        return True
    if digest != prev_digest:
        update_session_state(paths.state, idlePaneHash=digest, idlePaneAt=iso_now())
    return False
...
def _claude_completion_marker_present(capture: str) -> bool:
    if not capture:
        return False
    # Claude Code rotates through arbitrary whimsical completion verbs
    # ("Crunched for 4m 56s", "Cogitated for 19s", "Baked for 5m", ...).
    # Match ANY single capitalized word before a "for <duration>" timer —
    # a fixed verb whitelist silently misses completions (the pre-ladder
    # "orchestrator never sees the session finish" failure mode). Also
    # accept seconds-only durations for sub-minute turns.
    return bool(
        re.search(
            # \w (not [a-z]): the verbs include accented letters ("Sautéed",
            # "Flambéed") that ASCII ranges miss — 2026-07-03 field failure #3.
            r"(?m)^[^\w]{0,8}[A-Z]\w+\s+for\s+\d+(?:m(?:\s+\d+s)?|s)\b",
            capture,
        )
    )
...
def _capture_text_unfiltered(session: str, *, start: int) -> str:
    """Pane capture WITHOUT filter_input_box.

    Story 24-10 — the bmad-advanced-elicitation `(y/n)?` prompt renders
    inside the Claude input-box region (lines bracketed by `╭..╯`).
    `_capture_text` strips that region; the pane-watcher needs the raw
    text so it can match the prompt phrasing the inner agent is waiting
    on.
    """
    if not tmux_has_session(session):
        return ""
...
def _seconds_since(iso_timestamp: str) -> float:
    ...
    Returns +inf when the timestamp is empty or unparseable so that
    a missing prior-match record never debounces a fresh match.
    """
    if not iso_timestamp:
        return float("inf")
    try:
        parsed = datetime.strptime(iso_timestamp.replace("Z", "+00:00"), "%Y-%m-%dT%H:%M:%S%z")
    except ValueError:
        try:
...
def _maybe_auto_answer_elicitation_prompt(
    session: str,
    paths: SessionPaths,
    state: dict[str, object],
) -> None:
    """Story 24-10 pane-watcher entry point.

    Opt-in via `AUTO_ANSWER_ELICITATION=true` tmux env var (set by the
    spawn site when `overrides.autoAnswerElicitation` is true in the
    orchestration state doc). Matches the unfiltered pane tail against
    a conservative whitelist (`core.auto_answer.KNOWN_PROMPTS`); on a
    fresh match, sends `y` + Enter via the existing `tmux send-keys`
    path and records the event to the orchestration state doc's
    Action Log when `STATE_DOC_FILE` is also set.

    Debounce: the same literal prompt matched within
    `AUTO_ANSWER_DEBOUNCE_SECONDS` of the last send is treated as a
    re-render of the already-answered state, not a new prompt.
    """
    gate = tmux_show_environment(session, AUTO_ANSWER_ENV_VAR).strip().lower()
    state_doc = tmux_show_environment(session, AUTO_ANSWER_STATE_DOC_ENV_VAR).strip()

    capture = _capture_text_unfiltered(session, start=-50)
    matched = match_known_prompt(capture)
    if not matched:
        return

    if gate not in {"true", "1", "yes", "on"}:
        # AC3 Decision Point 4 — log-only on `autoAnswerElicitation: false`.
        # Interactive mode (gate off) still surfaces the would-have-answered
        # event so the operator sees what the watcher saw; useful as
        # low-friction beta testing of the whitelist before sleep-mode
        # runs depend on it. No `y` is sent.
        last_matched = str(state.get("autoAnswerSkippedPrompt") or "")
        last_at = str(state.get("autoAnswerSkippedAt") or "")
        if last_matched == matched and _seconds_since(last_at) < AUTO_ANSWER_DEBOUNCE_SECONDS:
            return
        now = iso_now()
        update_session_state(
            paths.state,
            autoAnswerSkippedPrompt=matched,
            autoAnswerSkippedAt=now,
        )
        if state_doc:
            _append_auto_answer_log(
                state_doc,
                session=session,
                prompt=matched,
                sent=None,
                reason="autoAnswerElicitation=false; whitelist match (skipped)",
                timestamp=now,
            )
        return

    # Gate is on — send `y` unless debounced.
    last_matched = str(state.get("autoAnswerLastPrompt") or "")
    last_at = str(state.get("autoAnswerLastAt") or "")
    if last_matched == matched and _seconds_since(last_at) < AUTO_ANSWER_DEBOUNCE_SECONDS:
        return

    send_out, send_code = run_cmd("tmux", "send-keys", "-t", session, "y", "Enter")
    if send_code != 0:
        # Send failed — record nothing; next poll will retry. Surfacing the
        # tmux error to stderr is the existing run_cmd contract.
        return

    now = iso_now()
    update_session_state(
        paths.state,
        autoAnswerLastPrompt=matched,
        autoAnswerLastAt=now,
    )
    if state_doc:
        _append_auto_answer_log(
            state_doc,
            ...
            prompt=matched,
            sent="y\\n",
            reason="autoAnswerElicitation=true; whitelist match",
            timestamp=now,
        )


def _append_auto_answer_log(
    state_doc_path: str,
    *,
    session: str,
    prompt: str,
    ...
    """Append one Action Log entry to the orchestration state-doc.

    Story 24-10 AC5. Entry shape matches the existing manual-entry
    convention (`- **[<ISO>]** ...`) so the log stays homogeneous.
    No-op if the file does not exist or the `## Action Log` section
    cannot be located — the watcher must never crash the polling
    loop over a log-write failure.
    """
    try:
        path = Path(state_doc_path)
        if not path.is_file():
            return
        text = read_text(path)
    except OSError:
    ...
    if sent is None:
        body = f'AUTO-ANSWER-SKIPPED: session={session} prompt="{prompt}" reason="{reason}"'
    else:
        body = (
            f'AUTO-ANSWER: session={session} prompt="{prompt}" '
            f'sent="{sent}" reason="{reason}"'
        )
    entry = f"- **[{timestamp}]** {body}\n"

    # Append at end of file. The state-doc template ends with a
    # frontmatter-free trailing region that already accumulates dated
    # entries; matching the existing convention beats trying to splice
    # into the `## Action Log` section heuristically.
    try:
...
def _check_prompt_visible(session: str) -> str:
    ...
    for line in reversed(capture.splitlines()[-8:]):
        ...
        if re.search(r"(❯|\$|#|%)\s*$", current):
            return "true"
    # Modern interactive Claude Code TUI: the session never returns to a
    # shell prompt, so the heuristics above can never fire (the input box
    # is "> ", often text-wrapped, with a multi-line status footer below).
    # While a turn is in flight the TUI renders an "esc to interrupt"
    # hint; when idle at the input box that hint disappears while the
    # footer chrome remains. Treat that as prompt-visible.
    if re.search(r"esc to interrupt", capture, re.IGNORECASE):
        return "false"
    if re.search(r"shift\+tab to cycle|bypass permissions|\? for shortcuts", capture):
        return "true"
    return "false"
