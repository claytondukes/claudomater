...
def cmd_build_state_doc(args: list[str]) -> int:
    ...
    # Story 24-10 — derive `overrides.autoAnswerElicitation` from
    # customInstructions. Sleep-mode runs (text contains "Sleep mode")
    # pre-authorize the pane-watcher to auto-answer known
    # bmad-advanced-elicitation y/n prompts inside spawned tmux
    # sessions. Default is false: interactive runs see the watcher in
    # log-only mode (records AUTO-ANSWER-SKIPPED entries; never sends
    # `y`).
    raw_custom_instructions = config.get("customInstructions", "")
    derived_auto_answer = (
        bool(overrides.get("autoAnswerElicitation", False))
        or ("sleep mode" in str(raw_custom_instructions).lower())
    )
    text = re.sub(
        r"(?m)^overrides:\n(?:(?:\s{2}.*\n)*)",
        "overrides:\n"
        ...
        f"  autoAnswerElicitation: {str(derived_auto_answer).lower()}\n",
        text,
    )
    custom_instructions = json.dumps(raw_custom_instructions)
