...
**SINGLE EXCEPTION (PR-mode done-gating, Story 1.1 postmortem 2026-07-14):**
in `step-03b-execute-finish` § E.7 — and ONLY there — the orchestrator flips
the current story's `development_status` entry from `review` to `done` after
the PR's CI checks are green and the Copilot loop (when enabled) has
converged. In PR mode the review bridge deliberately holds the entry at
`review`, and no session exists at convergence time to perform the flip.
Every other sprint-status write remains forbidden.

**WHO updates it:** The T-Mux sessions running dev-story, code-review, etc. (plus the single step-03b § E.7 flip above).
...
3. **Escalate, don't decide** — When uncertain, ask the user. Never answer your own escalation unless the active policy explicitly authorizes auto-answer for that trigger class; every resolution log line carries `resolved-by:` (user | auto-answer-policy | orchestrator-default) — see `escalation-triggers.md`
