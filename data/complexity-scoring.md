...
| Score | Level | Meaning | Model tier (create/dev/auto) |
|---|---|---|---|
| ≤ 3 | **Low** | High success probability | Sonnet (`claude-sonnet-5`) |
| 4–7 | **Medium** | Normal execution, moderate risk | Opus (`claude-opus-4-8`) |
| ≥ 8 | **High** | Consider longer timeouts, may need intervention | Fable (`claude-fable-5`) |

Complexity drives the **model** used for `create`/`dev`/`auto`. `review`/`retro`
always run top-tier regardless of complexity, and the tier table shifts down when
Fable's weekly limit is exhausted. See **`model-selection.md`** for the full
mapping, the Fable-usage fallback, and configuration.
