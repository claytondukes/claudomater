...
| Blocked mid-wait (you just scheduled a wakeup) | **RE-PERSIST** → Ensure `waitSession` is set in the state doc, run `orchestrator-helper marker heartbeat`, re-schedule the wakeup, end the turn | The hook allows paced waits (`waitSession` + fresh heartbeat); being blocked means one of them is missing/stale. Do NOT start polling faster |
