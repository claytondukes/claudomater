...
| `while true` / `sleep` loops in LLM context | Session crash, use the `check-session` wait ladder (`data/wait-ladder.md`) |
| Blocking `$scripts monitor-session` from the orchestrator | Exceeds the Bash tool time cap and dies mid-wait; use `check-session` + stepped `ScheduleWakeup` |
...
# STEP 2: Wait via the stepped ladder (see data/wait-ladder.md) — persist the
# wait, then ONE non-blocking check per ScheduleWakeup wake (300s/180s/120s...):
result=$("$scripts" check-session "$session_name" --json \
  --workflow create --story-key 5.3 --state-file "$state_file" \
  --started-at "$waitStartedAt" --timeout 60)
# final_state=="running" -> schedule next wakeup and end the turn; else continue:
...
**Context savings:** each check is one cheap call; the waiting happens between turns via `ScheduleWakeup`, so no tool call ever blocks on a running session.
