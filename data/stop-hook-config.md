...
- Check the binary exists: `test -x ~/.claude/skills/story-automator/scripts/story-automator`
- Ensure binary is executable: `chmod +x ~/.claude/skills/story-automator/scripts/story-automator`
...
### Heartbeat & the Paced-Wait Exemption

The heartbeat is the liveness signal for the **paced-wait exemption** (2026-07-14, Story 1.2 run postmortem): when the orchestrator ends its turn mid-wait per `wait-ladder.md` (non-empty `waitSession` in the state doc, marker heartbeat fresher than 15 minutes), the hook allows the stop — the scheduled wakeup is the pump, and blocking would cancel it and collapse the ladder into a tight poll loop. Once the heartbeat goes stale (≥ 15 min: the pacing loop died), blocking resumes so any stopping session picks up recovery. The orchestrator refreshes the heartbeat on every wake tick (`orchestrator-helper marker heartbeat`).
...
| Scenario | Action |
|----------|--------|
| `STORY_AUTOMATOR_CHILD=true` | `exit 0` → Always allow (child session; stamps `turnCompletedAt`) |
| No marker file | `exit 0` → Allow stop |
| Marker exists, `storiesRemaining=0` | `exit 0` → Allow stop |
| Marker exists, remaining > 0, marker pid alive elsewhere (bystander session) | `exit 0` → Allow stop (run is already driven) |
| Marker exists, remaining > 0, `waitSession` persisted + heartbeat < 15 min (paced wait) | `exit 0` → Allow stop (the scheduled wakeup is the pump) |
| Marker exists, remaining > 0, otherwise | Output JSON → Block stop with reason |
