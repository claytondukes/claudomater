...
Script: resolve the installed helper under `~/.claude/skills/story-automator/scripts/story-automator` (or the project-local `.claude/skills/story-automator/...` when present)

```bash
# ALWAYS use absolute path - relative paths break when directory changes
script="$PROJECT_ROOT/.claude/skills/story-automator/scripts/story-automator"
[ -x "$script" ] || script="$HOME/.claude/skills/story-automator/scripts/story-automator"
...
- `AI_AGENT` = `claude` or `codex` (used by story-automator tmux-wrapper, check-session, and monitor-session)
