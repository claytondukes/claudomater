...
def _tracked_text_files() -> list[Path]:
    ...
    try:
        ...
        pytest.fail(f"git is not available - the scan cannot enumerate tracked files: {exc}")
    except subprocess.CalledProcessError as exc:
        # capture_output swallows git's stderr, so a bare re-raise would
        # hide the one line that says why (e.g. "not a git repository")
        pytest.fail(
            "git ls-files failed - the scan needs a git checkout, not an "
            f"exported tree: {exc.stderr.strip() or exc}"
        )
    files = [
        path
