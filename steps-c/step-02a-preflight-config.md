...
- Step 2 completed.
- Variables available: `epic_id`, `epic_name`, `range_json`, `stories_json`, `selected_count`, `custom_instructions`, `init_log`.
...
Persist any preflight notes to `{outputFile}`. Update frontmatter with
the canonical preflight markers and `lastUpdated` (Story 24-7 — the
state document was created in Section 4, so this is the first
invocation site for `--append-array stepsCompleted=…`):

```bash
"{buildStateDoc}" orchestrator-helper state-update "$state_path" \
...
Close the init-log run marker (run-death trace — Story 1.1 postmortem,
2026-07-14): the state doc now exists, so record the handoff:
```bash
printf "[%s] launched: state=%s\n" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$state_path" >> "$init_log"
```
