...
Set status="IN_PROGRESS", log "Execution started", and record the
canonical preflight-finalize marker in the state-doc invariant ledger
(Story 24-7):
...
"{stateHelper}" orchestrator-helper state-update "{outputFile}" \
  ...
  --append-array stepsCompleted=step-02b-preflight-finalize
```
