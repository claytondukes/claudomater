...
"$scripts" check-session --help >/dev/null
...
### Resolve pane-watcher flags (Story 24-10)

Read `overrides.autoAnswerElicitation` from the state doc. When
true, every spawned tmux session gets `AUTO_ANSWER_ELICITATION=true`
+ `STATE_DOC_FILE=<state_file>` env vars, which the pane-watcher
inside every `check-session`/`monitor-session` status check (see
`src/story_automator/core/auto_answer.py`) uses to detect and
auto-answer known `bmad-advanced-elicitation` `(y/n)?` prompts.
Default (interactive runs) leaves the env var at `false`: the
watcher then runs in log-only mode (records `AUTO-ANSWER-SKIPPED`
entries; never sends `y`).
...
Every `tmux-wrapper spawn` invocation below appends
`"${auto_answer_flags[@]}"` to the args.
...
session=$("$scripts" tmux-wrapper spawn create {epic} {story_id} \
  ...
  "${auto_answer_flags[@]}")
"$scripts" orchestrator-helper state-update "$state_file" \
  --set waitSession="$session" --set waitKind=create --set waitAgent="$current_agent" \
...
`prompt`: `/story-automator resume`; `reason`: e.g. "polling create session for {story_id} (check #N)"),
...
session=$("$scripts" tmux-wrapper spawn dev {epic} {story_id} \
  ...
  "${auto_answer_flags[@]}")
"$scripts" orchestrator-helper state-update "$state_file" \
  --set waitSession="$session" --set waitKind=dev --set waitAgent="$current_agent" \
  --set waitStory="{story_id}" --set waitPolls=0 \
  --set waitStartedAt="$(date +%s)" --set waitTimeoutMin=60
```
...
**WAIT — non-blocking stepped ladder (full pattern: `data/wait-ladder.md`):** call
`ScheduleWakeup` (`delaySeconds`: 300 for check #1, then 180, then 120 repeating;
`prompt`: `/story-automator resume`; `reason`: e.g. "polling dev session for {story_id} (check #N)"),
then END the turn. Re-entry guard: if the state doc already has a non-empty `waitSession`
for this story/phase, skip the spawn above and start here. On each wake run exactly ONE
non-blocking check (NEVER the blocking `monitor-session`):
...
"$scripts" orchestrator-helper state-update "$state_file" \
  ...
  --set lastUpdated="$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  --append-array stepsCompleted=step-03-execute:${story_id}
